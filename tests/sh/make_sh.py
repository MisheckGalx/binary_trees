...
Data structure
 ...
 Weight: 5
 Project to be done in teams of 2 people (your team: Misheck Gogo, Brian Musakwa)
...
counter = 0
for r in dx.split("alex@/tmp/binary_trees$"):
	code = r.split("alex@/tmp/binary_trees$")[0].strip()
	if code.startswith('gcc'):
		if counter == 19:
			break
		if not os.path.isfile(f'{counter}-main.sh'):
			f = open(f"{counter}-main.sh", 'a')
		else:
			f = open(f"{counter}-main.sh", 'w')
		c = code.split(' ')[-1]
		code = code.replace(f'{counter}-main.c', f'./tests/{counter}-main.c')
		code = code.replace(c, f'./tests/executables/{counter}-main')
		f.write(code)
		f.close()
		counter+=1